...
class ChristmasPeriodTest {
    ...
    static Stream<Arguments> createPeriod() {
        return Stream.of(
                ...
                arguments(LocalDate.of(2023, 12, 26), false),
                arguments(LocalDate.of(2024, 12, 26), false)

        );
    }

    @DisplayName("주문 내역의 주문일이 크리스마스 기간이면 참을 반환한다")
    @ParameterizedTest
    @MethodSource("createPeriod")
    void isChristmasPeriod(LocalDate orderDate, Boolean expected) {
        Orders orders = createOrder(orderDate);
        ChristmasPeriod christmasPeriod = new ChristmasPeriod();

        Boolean eventPeriod = christmasPeriod.isEventPeriod(orders);
